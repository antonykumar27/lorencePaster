...
const CreateGallery = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    date: "",
    category: "events",
    orientation: "landscape", // 1. പുതിയ സ്റ്റേറ്റ് വാല്യൂ (Default: landscape)
    description: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    payload.append("orientation", formData.orientation); // 2. പെലോഡിലേക്ക് orientation ചേർത്തു
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* 3. ഒറിയന്റേഷൻ സെലക്ട് ചെയ്യാനുള്ള ഡ്രോപ്പ്ഡൗൺ ഇവിടെ നൽകി */}
              <div>
                <label className="block text-sm font-semibold mb-1">
                  Image Layout / Orientation *
                </label>
                <select
                  name="orientation"
                  value={formData.orientation}
                  onChange={handleChange}
                  className="w-full px-4 py-2 rounded-xl border bg-gray-50 dark:bg-slate-900 font-medium text-indigo-600 dark:text-indigo-400"
                >
                  <option value="landscape">
                    🌅 Horizontal (Landscape / Desktop View)
                  </option>
                  <option value="portrait">
                    📱 Vertical (Portrait / Mobile View)
                  </option>
                </select>
              </div>
            </div>
              ...
              <div className="border-2 border-dashed border-gray-300 dark:border-slate-700 rounded-2xl p-6 text-center transition-colors">
                {!imagePreview ? (
                  ...
                  <div className="relative inline-block w-full">
                    {/* 4. ഡൈനാമിക് പ്രിവ്യൂ ക്ലാസ്സ് ചേർത്തു: സെലക്ട് ചെയ്യുന്നതിന് അനുസരിച്ച് പ്രിവ്യൂ ബോക്സ് മാറും */}
                    <img
                      src={imagePreview}
                      alt="Preview"
                      className={`mx-auto rounded-lg border shadow-md transition-all object-cover ${
                        formData.orientation === "landscape"
                          ? "aspect-video max-h-64 w-auto"
                          : "aspect-[3/4] max-h-80 w-auto"
                      }`}
                    />
                    <button
                      type="button"
                      onClick={removeImage}
                      className="absolute top-0 right-0 md:-top-3 md:-right-3 bg-red-500 rounded-full p-1.5 text-white hover:bg-red-600 transition"
                    >
                      <X size={16} />
                    </button>
                  </div>
